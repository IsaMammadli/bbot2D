...
def run_simulation(fw, fv1, fv2):
    ...
    return df
nu_s = [0, -0.05, -0.25, -1.0, 0.25, 1.0]
#Trajectory (a)
fw  = lambda t: -1.0
nu = 0
fv1 = lambda t:  0.001*np.cos(fw(t)*t+nu*t)#+0.05*np.cos(0.1*fw(t)*t+1.2)
fv2 = lambda t: -0.001*np.sin(fw(t)*t+nu*t)#-0.02*np.sin(0.1*fw(t)*t+1.2)
...
# plt.tight_layout()
plt.savefig('../code_outputs/figures/doubleloops.pdf', dpi=300)